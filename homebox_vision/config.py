"""Centralized configuration for Homebox Vision Companion.

All environment variables use the HOMEBOX_VISION_ prefix to avoid
...
class Settings:
    ...
    All environment variables use the HOMEBOX_VISION_ prefix to ensure
    ...
    api_url: str = field(default_factory=lambda: os.environ.get(
        "HOMEBOX_VISION_API_URL", DEMO_API_URL
    ))
    ...
    server_host: str = field(default_factory=lambda: os.environ.get(
        "HOMEBOX_VISION_SERVER_HOST", "0.0.0.0"
    ))
    server_port: int = field(default_factory=lambda: int(os.environ.get(
        "HOMEBOX_VISION_SERVER_PORT", "8000"
    )))

    # Logging configuration
    log_level: str = field(default_factory=lambda: os.environ.get(
        "HOMEBOX_VISION_LOG_LEVEL", "INFO"
    ))
    ...
    def validate(self) -> list[str]:
        ...
        if not self.openai_api_key:
            issues.append(
                "HOMEBOX_VISION_OPENAI_API_KEY is not set. "
